...
# Agregamos un número aleatorio al final de la URL para forzar la actualización (anti-cache)
timestamp = int(time.time())

try:
    r_m = requests.get(f"https://api.checkwx.com/metar/{AERODROMOS}?nocache={timestamp}", headers=headers)
    r_t = requests.get(f"https://api.checkwx.com/taf/{AERODROMOS}?nocache={timestamp}", headers=headers)
    r_s = requests.get(f"https://api.checkwx.com/speci/{AERODROMOS}?nocache={timestamp}", headers=headers)
