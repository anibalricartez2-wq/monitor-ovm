...
def diff_angular(d1, d2):
    """Calcula la diferencia mínima entre rumbos (salto de 360°)."""
    diff = abs(d1 - d2)
...
def auditar(reporte, taf):
    ...
    if vr is not None and vt is not None:
        ...
        # Solo evaluamos el Criterio A si hay al menos 10kt en el reporte o en el pronóstico
        if vr >= 10 or vt >= 10:
            d_ang = diff_angular(dr, dt)
            if d_ang >= 60:
                alertas.append(f"🚩 CRIT A: Giro de {d_ang}° (Intensidad >= 10kt)")
        
        # Criterio B: Diferencia de intensidad >= 10kt (Independiente del giro)
        if abs(vr - vt) >= 10:
...
for i, icao in enumerate(AERODROMOS):
    try:
        ...
        # Consultas individuales para evitar caché
