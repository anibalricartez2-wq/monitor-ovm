...
def parse_viento(texto):
    ...
    # Captura rumbos (000-360), intensidad (2 o 3 dígitos) y ráfagas (G)
...
def auditar(reporte, taf):
    ...
    if vr is not None and vt is not None:
        ...
        # --- MODIFICACIÓN: Alerta si hay 10kt o más en cualquiera de los dos ---
        if d_ang >= 60 and (vr >= 10 or vt >= 10):
            alertas.append(f"🚩 CRIT A: Giro de {d_ang}° (Vto >= 10kt)")
        
        # Criterio B: Diferencia de intensidad >= 10kt
        if abs(vr - vt) >= 10:
            alertas.append(f"🚩 CRIT B: Dif. Int. {abs(vr-vt)}kt")
            
        # Criterio C: Ráfagas (10kt por encima del viento medio del TAF)
        if rr >= (vt + 10):
...
for i, icao in enumerate(AERODROMOS):
    try:
        ...
        with cols[i % 2]:
            # El título del recuadro cambia a ROJO si hay alertas
            ...
            with st.expander(f"📍 {icao} - {estado_label}", expanded=True):
                ...
                # Resaltamos si es un SPECI (CheckWX suele incluirlos como el reporte más reciente)
                if "SPECI" in metar:
                    st.warning(f"🔔 SPECI ACTIVO: `{metar}`")
                else:
                    st.markdown(f"**ACTUAL:** `{metar}`")
                
                # Listado de alertas detectadas
                if alertas:
                    for a in alertas:
                        st.error(a)
                elif "Sin datos" not in metar:
                    st.success("Operación dentro de parámetros.")
